...
void handleRoot() {
  ...
  html += "input[type=range]::-webkit-slider-thumb{-webkit-appearance:none; width:22px; height:22px; background:var(--red); border-radius:50%; cursor:pointer;}";
...
void handleConfig() {
  ...
  // ==========================================
  // BOTÃO DE ATUALIZAÇÃO CLOUD OTA (STATUS AUTOMÁTICO)
  // ==========================================
  html += "<div class='card'><h2 style='color:#E31E24;'>Manutenção</h2>";
  // O botão nasce desativado, cinza e esperando a checagem
  html += "<a id='btn-ota' href='#' style='display:block; background:#2a2a2a; color:#888; text-align:center; padding:15px; border-radius:6px; text-decoration:none; font-weight:bold; font-size:1.1rem; margin-bottom:5px; pointer-events:none;'>Verificando...</a>";
  html += "</div>";

  // Script invisível que lê o GitHub e muda o botão instantaneamente
  html += "<script>";
  html += "fetch('https://raw.githubusercontent.com/beAMPbr/beAMP-Firmwares/refs/heads/main/QUAD/versao.txt?t=' + Date.now())";
  html += ".then(r => r.text()).then(v => {";
  html += "  let btn = document.getElementById('btn-ota');";
  html += "  if(v.trim() !== '" + FIRMWARE_VERSION + "' && v.trim() !== '') {";
  html += "    btn.innerHTML = 'Novo firmware encontrado. Clique para atualizar.';";
  html += "    btn.href = '/update_cloud';";
  html += "    btn.style.background = '#E31E24';"; // Acende o botão de vermelho
  html += "    btn.style.color = '#FFF';";
  html += "    btn.style.pointerEvents = 'auto';"; // Libera o clique para o usuário
  html += "  } else {";
  html += "    btn.innerHTML = 'Firmware atualizado';";
  html += "  }";
  html += "}).catch(() => { document.getElementById('btn-ota').innerHTML = 'Firmware atualizado'; });";
  html += "</script>";
...
void setup() {
  ...
  if(wm.autoConnect(nomeAP.c_str())) {
      ...
      server.on("/update", HTTP_GET, []() {
        ...
        htmlOTA += "<body><h2>beAMP Firmware Update (TESTE 19:58)</h2><form method='POST' action='/update' enctype='multipart/form-data'>";
